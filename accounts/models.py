...
class Interessado(models.Model):
    ...
    cpf = models.CharField(
        max_length=14, null=True, unique=True, default="Somente numeros"
    )
...
class Feedback(models.Model):
    ...
    CHOICES = [["F", "Fácil"], ["M", "Moderado"], ["D", "Difícil"]]

    CHOICES2 = [["S", "Sim"], ["N", "Não"]]

    descricao = models.CharField(null=True, max_length=1000)
    scpt = MultiSelectField(null=True, choices=CHOICES2, max_choices=1, max_length=1)
    cadastrarr = MultiSelectField(
        null=True, choices=CHOICES, max_choices=1, max_length=1
    )
    visualizacao = MultiSelectField(
        null=True, choices=CHOICES, max_choices=1, max_length=1
    )
    localizacao = MultiSelectField(
        null=True, choices=CHOICES, max_choices=1, max_length=1
    )
    relatorio = MultiSelectField(
        null=True, choices=CHOICES2, max_choices=1, max_length=1
    )
